...
class TableSummaryEdit(QWidget):
    """One-line display for a table variable ({"columns": [...],
    "rows": [...]}): a summary ("3 columns x 5 rows") plus an edit icon
    that opens the spreadsheet TableEditorDialog (headers on top, the
    columns' values below)."""
    ...
    def __init__(self, table: dict, title: str = "Table", parent=None):
        ...
        self._table = _tables.coerce_table(table)
        self._title = title
        ...
        self.edit_button.setIcon(icon("edit", "text-light"))
        self.edit_button.setToolTip("Open the table editor\u2026")
    ...
    def _refresh(self) -> None:
        self.summary.setText(self._tables.summary(self._table))

    def _open_dialog(self) -> None:
        dialog = TableEditorDialog(self._table, self._title, self)
        if dialog.exec() == QDialog.Accepted:
            self._table = dialog.result_table()
            self._refresh()
            self.changed.emit(self._table)
